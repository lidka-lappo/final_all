...
//		cout<<" j: "<<j<<" i: "<<i<<" list_size "<<listZ.at(i).size()-1<<" k: "<<k<<" tmp: "<<tmp-1<<endl;
	...
	tmpname1<<names[ZS]<<AS<<"chains_of_production.txt";
	...
	cout<<" <- "<<names[listZ.at(1).at(s)]<<listA.at(1).at(s)<<" with frequency "<<f_direct<<endl;
	...
	cout<<"Other possible routes: "<<n_vectors<<endl;
	myfile<<"Other possible routes: "<<n_vectors<<endl;
	for(int lev =n_vectors; lev >0; lev --)
	{
		...
		cout<<"with frequency "<<listFreq.at(lev).at(pos)<<endl;
		myfile<<"with frequency "<<listFreq.at(lev).at(pos)<<endl;
		cout<<lev<<" : "<<names[listZ.at(lev).at(pos)]<<listA.at(lev).at(pos)<<"-->";
		myfile<<lev<<" : "<<names[listZ.at(lev).at(pos)]<<listA.at(lev).at(pos)<<"-->";
		...
		while(level>0)
		{
			cout<<names[listZ.at(level).at(pos)]<<listA.at(level).at(pos)<<"-->";
		...
		cout<<names[listZ.at(0).at(0)]<<listA.at(0).at(0)<<endl;
		myfile<<names[listZ.at(0).at(0)]<<listA.at(0).at(0)<<endl;
		cout<<"Best route is "<<best_route<<endl;
