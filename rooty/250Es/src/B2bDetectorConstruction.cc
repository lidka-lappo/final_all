...
void B2bDetectorConstruction::DefineMaterials()
{
  ...
  // define an Element from isotopes, by relative abundance
    ...
    G4Isotope* target_isotope = new G4Isotope("250Es", iz=99 , n=250 , a=250.078611*g/mole);
    G4Element* target_element  = new G4Element("pure 250Es", "250Es", ncomponents=1);
    target_element->AddIsotope(target_isotope, abundance= 100.*perCent);

    density = 8.84  * g/cm3;
    //fTargetMaterial  = new G4Material(name="250Es_material", density, ncomponents=1);

    fTargetMaterial  = new G4Material(name="250Es_material", density, ncomponents=1, state = kStateGas);
