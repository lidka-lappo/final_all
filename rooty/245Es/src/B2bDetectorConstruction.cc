...
void B2bDetectorConstruction::DefineMaterials()
{
  ...
  // define an Element from isotopes, by relative abundance
    ...
    G4Isotope* target_isotope = new G4Isotope("245Es", iz=99 , n=245 , a=245.071192*g/mole);
    G4Element* target_element  = new G4Element("pure 245Es", "245Es", ncomponents=1);
    target_element->AddIsotope(target_isotope, abundance= 100.*perCent);

    density = 8.84  * g/cm3;
    //fTargetMaterial  = new G4Material(name="245Es_material", density, ncomponents=1);

    fTargetMaterial  = new G4Material(name="245Es_material", density, ncomponents=1, state = kStateGas);
