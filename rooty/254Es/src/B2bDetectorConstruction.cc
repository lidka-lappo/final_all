...
void B2bDetectorConstruction::DefineMaterials()
{
  ...
  // define an Element from isotopes, by relative abundance
    ...
    G4Isotope* target_isotope = new G4Isotope("254Es", iz=99 , n=254 , a=254.088024337*g/mole);
    G4Element* target_element  = new G4Element("pure 254Es", "254Es", ncomponents=1);
    target_element->AddIsotope(target_isotope, abundance= 100.*perCent);

    density = 8.84  * g/cm3;
    //fTargetMaterial  = new G4Material(name="254Es_material", density, ncomponents=1);

    fTargetMaterial  = new G4Material(name="254Es_material", density, ncomponents=1, state = kStateGas);
