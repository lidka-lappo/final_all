...
void B2bDetectorConstruction::DefineMaterials()
{
  ...
  // define an Element from isotopes, by relative abundance
    ...
    G4Isotope* target_isotope = new G4Isotope("244Es", iz=99 , n=244 , a=244.070881*g/mole);
    G4Element* target_element  = new G4Element("pure 244Es", "244Es", ncomponents=1);
    target_element->AddIsotope(target_isotope, abundance= 100.*perCent);

    density = 8.84  * g/cm3;
    //fTargetMaterial  = new G4Material(name="244Es_material", density, ncomponents=1);

    fTargetMaterial  = new G4Material(name="244Es_material", density, ncomponents=1, state = kStateGas);
